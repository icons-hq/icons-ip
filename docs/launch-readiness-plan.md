...
- **커머스·티케팅 DB**: `orders`/`payments`/`refunds`/`ticket_types`/`tickets` 스키마와 원자적 RPC가 존재한다. `place_order`·결제 확정·만료 복원은 앱에 연결됐고 티켓 예약·검표와 사용자 취소는 후속 이슈 범위다.
...
| **Commerce** | #89 실장바구니·#90 체크아웃 완료 → #91 주문 내역, #92 취소·청약철회, #93 어드민 주문 콘솔 · #94 실재고 관리 | #91·#92는 #90 뒤 |
