...
- **커머스·티케팅 DB**: `orders`/`payments`/`refunds`/`order_cancellation_requests`/`ticket_types`/`tickets` 스키마와 원자적 RPC가 존재한다. 커머스 흐름과 관리자 회차 콘솔은 앱에 연결됐고 티켓 예약·내 티켓·환불·검표는 후속 이슈 범위다.
...
| **Ticketing** | #96 회차 콘솔 완료 → #54 예매 플로우 → #95 내 티켓·환불, #97 검표 | #54는 #88 뒤 |
