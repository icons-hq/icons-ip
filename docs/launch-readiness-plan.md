...
- **인앱 알림**: 본인 RLS 알림함·unread 벨·마이페이지 진입점, 팔로우 IP별 드롭/이벤트 설정, IP·예정 이벤트 CTA가 연결됐다. 주문 상태·카드팩 발급·runtime staff 카탈로그 INSERT가 멱등 trigger로 발급하며 이메일·푸시는 제외한다(#104).
...
| 알림 | 인앱 알림함·unread 벨·IP별 드롭/이벤트 설정·IP/예정 이벤트 CTA 완료(#104) | 어드민 공지 발송 #105 |
...
| **Notifications** | #104 알림함·IP 알림 완료 → #105 어드민 공지 발송 | #105 착수 가능 |
...
남은 agent-executable 권장 착수 순서: ① #105 어드민 공지 발송 ② Community·Admin Ops. #137 탈퇴 실행은 #102 탈퇴 보존 정책 뒤에 착수하고, #17 소셜 로그인·#87 라이브 결제·#102 탈퇴 보존 정책은 별도 human gate로 닫는다.
