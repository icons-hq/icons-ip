...
- **커머스·티케팅 DB**: `orders`/`payments`/`refunds`/`ticket_types`/`tickets` 스키마와 원자적 RPC(`place_order`·`reserve_tickets`·`confirm_*`·`cancel_order`·`refund_ticket_order`·`check_in_ticket`)는 존재 — **앱 호출부가 0건**인 상태.
...
| 영역 | 현황 | 갭 → 이슈 |
|---|---|---|
| 장바구니 | `CartProvider` 로컬 카운터뿐 | 실카트·병합 #89 |
| 체크아웃·결제 | `app/api` 부재, 토스 연동 0건 | 인프라 #88(#87), 체크아웃 #90 |
| 주문 | 내역·상세·취소 UI 전무 | #91, #92 |
...
| C. 인프라 부재 | 토스 웹훅 수신부 #88, 실재고 조정 #94 |
...
| Track | 이슈 | 의존성 |
|---|---|---|
| **Payments** | #87 [human] 토스 상점 계약·키, #88 결제 인프라(웹훅·승인 경로) | #88은 테스트 키로 즉시 착수 가능 |
| **Commerce** | #89 실장바구니 → #90 체크아웃 → #91 주문 내역, #92 취소·청약철회, #93 어드민 주문 콘솔 · #94 실재고 관리 | #90은 #88·#89 뒤 |
...
권장 착수 순서: ① Payments(#88, 테스트 키) + 병렬로 Rewards Ops·Admin Ops·Account ② Commerce 체인 ③ Ticketing 체인 ④ Notifications → 공지 발송.
