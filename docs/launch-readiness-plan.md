...
- **어드민**: staff/admin 게이트, 카탈로그 upsert 4종, 신고 처리·포스트 숨김, 역할 부여·회수, 주문·배송·환불, 멱등 실재고 입고·보정, 티켓 회차·가격·정원, 실데이터 매출 대시보드 — 전부 audited.
- **커머스·티케팅 DB**: `orders`/`payments`/`refunds`/`order_cancellation_requests`/`ticket_types`/`tickets`/`ticket_cancellation_requests` 스키마와 원자적 RPC가 존재한다. 커머스 흐름, 관리자 회차 콘솔, 공개 회차 선택·멱등 티켓 예약·토스 결제·웹훅 QR 발급, 내 티켓·예매 전체 취소/환불이 앱에 연결됐고 검표는 후속 이슈 범위다.
...
| 티켓 예매 | 공개 상세→회차/수량 선택→10분 선점→토스 결제→웹훅 QR 발급→내 티켓·예매 전체 취소/환불 연결(#95) | 검표 #97, production 실결제 #87 |
...
| 유형 | 갭 → 이슈 |
|---|---|
| A. 스키마만 있고 어드민 경로 없음 | 카드풀·확률·풀 바인딩 #98, 뽑기권 정책 #99, 게임 등록 #100 |
| B. RPC는 있는데 UI 없음 | 검표 #97 |
...
| **Ticketing** | #96 회차 콘솔·#54 예매 플로우·#95 내 티켓/환불 완료 → #97 검표 | production 실결제는 #87 human gate |
...
권장 착수 순서: ① Ticketing 체인 ② Rewards Ops·Admin Ops·Account ③ Notifications → 공지 발송. 라이브 결제 전에는 #87을 별도 human gate로 닫는다.
