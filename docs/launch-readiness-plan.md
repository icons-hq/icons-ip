...
- **커머스·티케팅 DB**: `orders`/`payments`/`refunds`/`order_cancellation_requests`/`ticket_types`/`tickets` 스키마와 원자적 RPC가 존재한다. 커머스 흐름, 관리자 회차 콘솔, 공개 회차 선택·멱등 티켓 예약·토스 결제·웹훅 QR 발급이 앱에 연결됐고 내 티켓·환불·검표는 후속 이슈 범위다.
...
| 티켓 예매 | 공개 상세→회차/수량 선택→10분 선점→토스 결제→웹훅 QR 발급 연결 | 내 티켓·환불 #95, 검표 #97, production 실결제 #87 |
...
| **Ticketing** | #96 회차 콘솔·#54 예매 플로우 완료 → #95 내 티켓·환불, #97 검표 | production 실결제는 #87 human gate |
