...
- **어드민**: staff/admin 게이트, 카탈로그 upsert 4종, 카드풀 운영 기간·등급별 확률·카드 풀 바인딩, 뽑기권 발급 정책(`/admin?section=policy`)과 PII-free 발급/사용 가능/개봉/회수 집계, 신고 처리·포스트 숨김, 역할 부여·회수, 주문·배송·환불, 멱등 실재고 입고·보정, 티켓 회차·가격·정원·현장 검표, 마스킹 회원 검색·명시적 상세·계정 정지/해제, 실데이터 매출 대시보드 — 전부 audited.
...
| D. 도메인 자체 부재 | 회원 조회·제재 #111 완료. 아트워크 업로드 #112, 카탈로그 보관 #113, 배너·공지·큐레이션 #114 |
...
| **Admin Ops** | #111 회원 조회·제재 완료. #112 아트워크 업로드, #113 카탈로그 보관, #114 배너·공지·큐레이션 | #112~#114 즉시 착수 가능 |

남은 agent-executable 권장 착수 순서: Admin Ops #112→#113→#114. #137 탈퇴 실행은 #102 탈퇴 보존 정책 뒤에 착수하고, #17 소셜 로그인·#87 라이브 결제·#102 탈퇴 보존 정책은 별도 human gate로 닫는다.
