...
- **커머스·티케팅 DB**: `orders`/`payments`/`refunds`/`order_cancellation_requests`/`ticket_types`/`tickets` 스키마와 원자적 RPC가 존재한다. `place_order`·결제 확정·만료 복원·본인 주문 내역/상세·배송 전 청약철회 요청·관리자 승인/거절/재정합화는 앱에 연결됐고 티켓 예약·검표는 후속 이슈 범위다.
...
| **Commerce** | #89 실장바구니·#90 체크아웃·#91 주문 내역·#92 취소/청약철회·#93 어드민 주문 콘솔 완료 → #94 실재고 관리 | #94 즉시 착수 가능 |
...
권장 착수 순서: ① Commerce의 실재고 운영 #94 ② Ticketing 체인 ③ Rewards Ops·Admin Ops·Account ④ Notifications → 공지 발송. 라이브 결제 전에는 #87을 별도 human gate로 닫는다.
