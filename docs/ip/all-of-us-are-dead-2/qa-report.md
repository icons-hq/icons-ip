...
2. 교실 문까지 이동해 `문 잠그기`가 거리 안에서만 나타나는 것을 확인했다.
...
6. 화재문을 잠그고 마지막 종을 울렸다.
7. 종을 울린 뒤 멈춰 약 3.3초 안에 붙잡힘 화면과 `ch1_post_bell_safe` 저장을 확인했다.
8. `다시 시도`로 안전 지점에 복원한 뒤 달려 Chapter exit에 도달했다.
...
| `npm test` | 309 files passed, 1 skipped / 3,115 tests passed, 1 skipped |
