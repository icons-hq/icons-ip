...
- **커머스 코어**: 장바구니 병합, `place_order` 원자적 재고 선점, 15분 pending 만료, 굿즈 provider-neutral attempt/claim/finalizer, 주문 내역·상세. 실제 Korpay adapter와 rollout gate는 #207 전까지 OFF다.
...
6. **[#206](https://github.com/icons-hq/icons-ip/issues/206)·[#207](https://github.com/icons-hq/icons-ip/issues/207)** — #205의 굿즈 seam은 provider 기본 OFF로 구현됐다. 남은 티켓 checkout 전환과 Korpay dark deploy·controlled canary 증거 전에는 신규 공개 결제를 열지 않는다.
...
- #205에서 굿즈 checkout을 공통 seam으로 옮기고 실제 provider를 기본 OFF로 닫았다. #206이 티켓 checkout을 옮길 때까지 Toss는 티켓 transitional compatibility와 기존 거래 정리에만 남긴다. 신규 Toss live key·신규 공개 판매를 활성화하지 않는다.
