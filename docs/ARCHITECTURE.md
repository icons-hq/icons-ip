...
- 옵션 expand 단계의 `goods_variants`가 ICONS 할당 재고의 진실원이며 `goods.stock_qty`는 옵션 합계 캐시다. 기존 재고 차감·복원·조정 RPC는 기본 옵션에 위임하고, 신규 굿즈는 기본 옵션을 자동 생성한다. 장바구니·주문 품목 참조와 다중 옵션 편집은 migrate 단계에서 전환한다.
