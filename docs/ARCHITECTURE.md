...
- `private.account_deletion_requests` / `private.account_action_fences` — self-only 탈퇴 요청과 신규 구매·예매·작성·개봉·게임·프로필/마케팅 변경 fence. subject UUID는 hard delete 전 처리 locator로 private에만 두고 공개 RPC는 blocker 코드·건수·복구 경로와 opaque phase만 반환한다. Phase 1 activation control은 기본 OFF다.
- `private.account_deletion_legal_snapshots` — 주문·결제·환급·티켓의 allowlist 필드와 법적 근거·보존 만료·접근 목적만 담는 분리 원장. 주소·이메일·DOB·provider raw·payment key는 복제하지 않으며 Data API 역할과 일반 service role 직접 접근을 모두 회수한다.
...
- **`preview_my_account_deletion` / `request_my_account_deletion` / `get_my_account_deletion_status`** — target user ID를 받지 않고 `auth.uid()`만 사용하는 self-only seam이다. 정확한 확인 문구와 UUID idempotency key를 요구하고 진행 주문·취소·legacy pending 결제·provider-neutral 결제 시도·환급·유효 티켓·staff 권한을 fail closed로 평가한다. 최초 요청 transaction에서 allowlist legal snapshot과 write fence를 함께 만들며, 기존 거래의 webhook·환급·배송 정합화 UPDATE는 막지 않는다. 성공 상태도 `awaiting_notification`까지만 표시하고 #191 intent, Storage/DB/Auth hard delete, #215 tombstone 전에는 완료를 반환하지 않는다.
