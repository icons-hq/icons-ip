...
- `order_shipments` (id, order_id, origin_id, origin_name_snapshot, shipping_fee, shipping_fee_snapshot, status `ready|shipping|delivered|canceled`, carrier, tracking_number, shipped_at, delivered_at, exported_at) / `order_shipment_items` (order_id, shipment_id, order_item_id) — 출고지별 배송 건과 주문 품목의 연결. 운송장·배송비·배송 시각을 건별로 보존한다.
...
- 환불: `refunds` 완료 기록 + 재고 원복은 RPC가 담당한다. 웹훅은 청약철회 요청을 완료할 수 없고, 해당 요청은 staff 승인 뒤 fresh GET 전체 검증 경로에서만 종결한다. 접수·승인은 `order_withdrawal_deadline_passed`로 주문의 `delivered_at`과 사유별 기한을 검사한다. 이 시각은 모든 배송 건이 완료됐을 때의 마지막 도착 시각이며, 배송 건별 독립 클레임 기산점을 검사하는 구현은 아니다.
