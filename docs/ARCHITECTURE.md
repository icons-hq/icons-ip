...
| Last Bell 스토리 캠페인 | `/games/prototype-last-bell`은 local QA 전용이고 구매권을 만들지 않는다. 별도 gate의 `/experiences/all-of-us-are-dead/last-bell`은 `VerifiedRunHost`와 service-only RPC로 run·수집·출구를 검증한다. 완료 CTA와 계정 구매권 조회는 같은 gate의 전용 `/experiences/all-of-us-are-dead/last-bell/store`에 머물며 prototype popup 권위에 의존하지 않는다 | `app/games/prototype-last-bell`, `app/experiences/all-of-us-are-dead/last-bell`, `app/api/experiences/last-bell`, `lib/experiences/last-bell`, `supabase/migrations/20260825023830_last_bell_verified_runs_and_entitlements.sql`, `supabase/migrations/20260826034850_last_bell_claim_all_completed_guest_runs.sql` |
