...
| 쿠폰·회원 등급 | 카트 쿠폰 적용/해제(코드 입력·보유 쿠폰), `place_order` 안에서 확정되는 서버 할인과 주문 취소 시 복구, 마이 쿠폰함, 구매 실적 기반 4단 등급 산정과 등급 혜택 쿠폰 자동 발급, 어드민 쿠폰 콘솔 | `app/cart/coupon-actions.ts`, `app/my/coupons/*`, `lib/coupons*`, `lib/loyalty.ts`, `supabase/migrations/20260831090000_coupon_domain.sql`, `supabase/migrations/20260831090100_loyalty_grades.sql` |
...
- `ips` (id, title, sub, vertical_key, glyph, bg, tagline, synopsis, featured, published_at, archived_at, fans/goods/cards 집계)
...
- `order_shipments` (id, order_id, origin_id, origin_name_snapshot, shipping_fee, shipping_fee_snapshot, status `ready|shipping|delivered|canceled`, carrier, tracking_number, shipped_at, delivered_at, exported_at) / `order_shipment_items` (order_id, shipment_id, order_item_id, qty) — 출고지별 배송 건과 주문 품목·수량의 연결. 운송장·배송비·배송 시각을 건별로 보존하며, 클레임 접수는 실제 배송 완료된 수량과 주문 품목 수량을 대조한다.
- `coupons` (code PK, name, discount_type `fixed|percent`, discount_value, max_discount_amount, min_subtotal, starts_at/ends_at, issue_limit/issued_count, grade_benefit, status `active|archived`) / `user_coupons` (id, coupon_code, user_id, status `active|used`, issued_source `code_entry|grade_benefit|admin_grant`, issued_at, expires_at, used_at, used_order_id, unique(coupon_code, user_id)) / `coupon_redemptions` (id, user_coupon_id, coupon_code, user_id, order_id, discount_amount, status `applied|released`, applied_at, released_at, release_reason) / `cart_coupon_selections` (user_id PK, user_coupon_id) — **주문 금액에 개입하는 유일한 프로모션 수단**이다. 만료는 상태 배치가 아니라 발급 시점 `expires_at` 스냅의 파생으로 읽고, 부분 유니크 인덱스가 주문당 유효 쿠폰 1장과 보유 쿠폰당 유효 사용 1건을 스키마에서 막는다. 카트 선택은 `user_id`가 PK라 "장바구니 쿠폰 1장"이 스키마에 새겨진다. 할인액은 `orders.discount_total`에 명시 저장하고 `total`은 할인이 이미 반영된 최종 청구액이다 — 정산·환불이 역산하지 않는다. 주문 취소는 사용 원장을 지우지 않고 `released`로 남기며 보유 쿠폰을 `active`로 되돌린다(재발급이 아니다).
...
- `order_cancellation_requests` (id, order_id, requested_by, status, decision, collection_policy `legacy|origin`, collection_required, provider 상태 코드, 시각) — 사용자 요청과 운영 결정의 durable 원장. 회수 정책 전환은 기존 금융 결과나 완료 사실을 덮어쓰지 않는다.
...
- `fulfillment_origins`는 배송 정책·택배사·반품 주소·출고 마감·내보내기 열 설정을 갖는다. 굿즈별 출고지 정책 따름·무료·개별 배송비를 서버가 계산하고 주문별 출고지 스냅샷으로 보존한다. `order_shipments`와 품목 연결이 운송장·발송·도착·출고지시 전달의 진실원이다. 주문의 구 단일 운송장 컬럼은 쓰기가 봉인됐으며 읽기 호출처도 배송 건으로 전환했다. 일부 출고 후 주문은 배송중이고 모든 건의 도착과 철회 기간 경과가 자동 거래확정의 조건이다.
- 배송 전이는 ID만 담은 `order_shipment_email_jobs`를 같은 트랜잭션에 기록한다. 매분 cron은 최대 25건을 claim하고 5건씩 발송하며 lease·재시도·결과 불명 검토 상태를 보존한다. 배송 건별 이메일 멱등 키를 사용하고 내부 큐 상태는 관리자 주문 상세에만 제공한다.
...
- **`place_order(user_id, address, checkout_key, payment_method)`** — 결제 환경·인증·온보딩·production 검토 권한을 확인한 Server Action만 service role로 호출하며 브라우저 롤에는 execute를 열지 않는다. DB가 장바구니·상위 IP·굿즈·선택 옵션을 정해진 순서로 잠그고 게시/보관·판매 제한·옵션 재고를 재검증한다. 옵션 재고 차감, 주문 당시 가격·굿즈/옵션 이름·코드·유형·IP를 고정한 `orders`/`order_items`(`pending`), 출고지별 `order_shipments`와 품목 연결을 같은 트랜잭션에서 만든다. **쿠폰 할인도 여기서만 확정한다** — 선택 옵션 가격의 소계로 쿠폰을 다시 평가해(만료·최소 주문 금액·중복 사용·정률 상한, 결제사 최소 결제액 하한) `user_coupons`를 `used`로 옮기고 `coupon_redemptions`를 남긴 뒤 출고지별 배송비를 포함한 `orders.total`·`discount_total`을 확정한다. 카트에서 통과한 쿠폰이 결제 시점에도 유효하다고 가정하지 않는다.
...
- **발주확인·배송 전이·주문 검색** — `admin_update_order_status`의 `paid → confirmed`는 주문 단위다. 신규 배송 전이·운송장 정정은 staff를 DB에서 재확인하는 `admin_update_shipment_status`·`admin_update_shipment_tracking`이 배송 건 UUID로 처리하고 감사한다. `ready → shipping`에는 활성 택배사·운송장이 필요하며 `shipping → delivered` 뒤 전체 주문 상태를 집계한다. 구 주문 단위 배송 전이·운송장 RPC는 배송 건이 하나인 주문만 전달하고 혼합 주문은 `shipment_reference_required`로 거절한다. `admin_search_orders`는 주문/구매자/상태/KST 기간·발주확인 시점(`p_confirmed_before`)을 DB에서 페이지 처리하며 배송은 배열로 반환한다.
