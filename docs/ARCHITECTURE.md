...
- `goods_variants`가 옵션 가격·ICONS 할당 재고의 진실원이며 `goods.stock_qty`는 활성 옵션 합계 캐시다. 옵션이 없는 굿즈도 기본 옵션 한 개를 갖고 장바구니·주문 품목은 `variant_id`를 필수 참조한다. 최대 2축·100조합 편집은 굿즈 메타데이터·배송·게시 상태와 함께 audited RPC로 저장한다. 재고를 수정하지 않은 편집은 도중 구매 차감을 보존하고 직접 재고 변경은 낙관적 충돌 검사로 보호한다. 취소·환불은 원래 옵션으로 한 번만 복원하며 교환 재출고는 선택 옵션의 재고를 잠그고 차감한다.
...
- **`place_order(user_id, address, checkout_key)`** — 결제 환경·인증·온보딩·production 검토 권한을 확인한 Server Action만 service role로 호출하며 브라우저 롤에는 execute를 열지 않는다. DB가 장바구니와 굿즈를 잠근 뒤 재고 검증·차감, 주문 당시 가격·이름·유형·IP를 고정한 `orders`/`order_items` 생성(`pending`)을 한 트랜잭션에서 수행한다. **쿠폰 할인도 여기서만 확정한다** — 카트에 선택된 쿠폰을 같은 트랜잭션에서 다시 평가해(만료·최소 주문 금액·중복 사용·정률 상한, 결제사 최소 결제액 하한) `user_coupons`를 `used`로 옮기고 `coupon_redemptions`를 남긴 뒤 `orders.total`·`discount_total`을 확정한다. 카트에서 통과한 쿠폰이 결제 시점에도 유효하다고 가정하지 않는다.
...
- 환불: `refunds` 완료 기록 + 재고 원복은 RPC가 담당한다. 웹훅은 청약철회 요청을 완료할 수 없고, 해당 요청은 staff 승인 뒤 fresh GET 전체 검증 경로에서만 종결한다. 현재 배송·수령 시각이 없으므로 법정 7일을 앱이 자동 판정하지 않는다.
