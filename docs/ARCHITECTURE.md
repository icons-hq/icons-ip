...
- `ips.published_at`(nullable)이 IP 게시 상태의 진실원이다 — `archived_at`이 있으면 보관, 없고 `published_at`도 없으면 초안(등록 중·비공개), 있으면 공개. 신규 IP는 초안으로 시작하고 `admin_upsert_ip(target_publish)` 또는 `admin_set_ip_published`로만 전환하며 `admin.ip.published`/`admin.ip.unpublished`로 감사한다. 공개 카탈로그 로더(`lib/catalog.ts`)와 검색 RPC는 초안 IP와 그 소속 굿즈·카드·이벤트를 보관 필터와 같은 층(앱 로더·security definer RPC)에서 제외하고, RLS·FK는 그대로다. 보관에서 복원하면 이전 게시 상태로 돌아온다(도입 전 보관분은 게시 이력이 없어 초안).
