...
| 결제 | **Provider-neutral gateway + Korpay** | 굿즈·티켓 공통 seam과 Korpay adapter가 연결됐고 목적별 public gate는 기본 OFF다. Toss는 기존 거래의 조회·취소·웹훅만 남긴다 |
...
## 9. 결제 통합 (Korpay 신규 결제와 Toss legacy)
...
- 프리뷰의 굿즈·티켓 신규 checkout은 Fake로만 검증하며 Korpay 실자격 증명과 canary user ID를 넣지 않는다. 현재 남아 있는 legacy Toss 환경변수는 신규 checkout을 열지 않고, 알려지지 않은 거래는 provider 호출 전에 거부한다. Production에서도 신규 Toss live checkout이나 Toss 실결제 canary를 활성화하지 않고 `provider=toss`로 확인된 기존 두 거래의 조회·취소·웹훅만 유지한다. Korpay 계약 완료와 현재 자격 증명 사용 가능 상태는 2026-08-14 사용자 확인으로 확정했으며, #207 dark deploy·controlled canary 뒤 필요한 목적만 연다. 이 확인은 공급사 서면 증거나 19+ 유한 실물 쿠지 승인 범위를 대체하지 않는다. 기존 Toss 거래가 공급사 콘솔과 내부 원장에서 모두 종결된 뒤에만 Toss runtime과 server secret 제거를 별도 PR로 수행한다.
...
- **결제 provider rollout** — provider-neutral 원장과 굿즈·티켓 `PaymentGateway` seam, Korpay 인증·승인 adapter는 구현됐지만 목적별 public gate는 기본 OFF다. #207 controlled canary와 운영 readback 뒤 필요한 목적만 열며 Toss는 기존 거래 정리 전용으로 유지한다(§9, #204~#208).
