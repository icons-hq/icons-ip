...
| 결제 | **Provider-neutral gateway** | 굿즈는 공통 seam으로 이동해 provider gate가 OFF이고, 티켓만 #206 전환까지 Toss checkout을 호환 유지한다. 신규 목표는 Korpay이며 전환 뒤 Toss는 기존 거래의 조회·취소·웹훅만 남긴다 |
...
- 굿즈 checkout은 `prepare_goods_payment_attempt → bind nonce → claim_goods_payment_attempt → PaymentGateway.confirm → finalize_goods_payment_attempt`로 이동했다. attempt와 order를 고정 순서로 잠그고 owner·amount·KRW·스냅샷·취소/fence를 재검증하며, callback nonce 원문과 provider raw는 DB에 저장하지 않는다. `confirming | unknown | needs_review`는 만료 sweep이 재고를 해제하지 않는다. 실제 provider가 없는 runtime은 #207까지 OFF다. 티켓은 #206 전환 전까지 아래 Toss 호환 경로를 유지하므로 웹훅의 `unknown_compatibility` 예외도 그때까지 남는다. 두 checkout 전환 뒤에는 이 상태에서 provider를 호출하지 않는 known-only 테스트로 교체한다.

- 기존 Toss 클라이언트: 티켓만 #206 전환 전까지 결제위젯과 `ticket_<uuid>` 목적 식별자를 유지한다(`lib/payments/toss.ts`). 굿즈 UI와 주문 생성 action은 Toss key나 staff review override로 열리지 않는다.
- 티켓 호환 승인: successUrl 콜백이 **`/api/payments/confirm`** 을 호출 → 본인 소유·pending·미만료·금액 일치를 검증한 뒤 토스 승인 API를 호출하고 `payments`에 `pending`으로 기록한다. **승인 성공은 UX 반영용이다.**
...
- 굿즈 흐름: ① provider gate 확인 뒤 `pending` 주문·재고 선점 → ② 공통 attempt 준비와 nonce digest bind → ③ session-independent callback을 원자 claim → ④ gateway의 공통 outcome을 DB finalizer가 검증·확정 → ⑤ `approved`만 주문/결제를 paid로 만들고, 모호 상태는 재고·fulfillment를 보류한다. 티켓은 #206 전까지 기존 Toss payment claim→웹훅 확정 흐름을 유지한다.
...
- **결제 provider 전환** — provider-neutral 원장과 굿즈 `PaymentGateway` seam은 구현됐지만 실제 provider는 기본 OFF다. #206 티켓 전환과 #207 Korpay dark deploy 뒤 Toss를 기존 거래 정리 전용으로 축소한다(§9, #204~#207).
