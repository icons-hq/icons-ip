...
| 결제 | **Provider-neutral gateway** | 굿즈·티켓이 공통 seam으로 이동했고 provider gate는 OFF다. 신규 목표는 Korpay이며 Toss는 기존 거래의 조회·취소·웹훅만 남긴다 |
...
- 굿즈와 티켓 checkout은 각각 `prepare_*_payment_attempt → bind nonce → claim_*_payment_attempt → PaymentGateway.confirm → finalize_*_payment_attempt`로 이동했다. attempt와 주문/예매를 고정 순서로 잠그고 owner·amount·KRW·스냅샷·취소/fence를 재검증하며, callback nonce 원문과 provider raw는 DB에 저장하지 않는다. `confirming | unknown | needs_review`는 만료 sweep이 재고·정원을 해제하지 않는다. 실제 provider가 없는 runtime은 #207까지 OFF다.
...
- 결제·환급 재조정은 자동 scan/retry가 아니라 검토된 한 건만 지정하는 내부 route다. 분리된 `PAYMENT_RECONCILIATION_SECRET` 자체와 exact bearer가 모두 16~128자 URL-safe 형식이어야 하며, PII 없는 opaque `caseRef`만 받는다. actor는 요청값을 신뢰하지 않고 서버가 `payment_reconciliation_service_v1`으로 고정해 DB audit에 operation·target·outcome을 남긴다. 관리자 아트워크 `CRON_SECRET`으로는 호출할 수 없다. Provider adapter readiness는 `order`·`ticket` 목적별 신규 checkout rollout gate와 분리한다. 일반 굿즈 staff canary와 티켓 판매를 따로 활성화·rollback하며, 판매 pause는 해당 목적의 새 reserve·prepare·provider session 생성을 닫는다. 이미 durable한 attempt의 known opaque order+nonce callback과 명시적 reconcile/refund는 readiness 경계에서 drain한다. 알 수 없는 order·nonce는 DB claim에서 provider 호출 전에 거부한다. #206 dark deploy에서는 secret을 provision하지 않아 route가 401로 닫히며 Preview/CI에는 어느 실 결제 자격 증명도 넣지 않는다.
- Vercel 환경변수는 sensitive로 유지한다. GitHub Actions는 값을 복호화할 수 없는 `vercel pull` + prebuilt 경로를 쓰지 않고 Vercel 원격 build를 요청한다. `prebuild` guard는 공통 필수 변수와 Production의 Toss known-only server key·기존 cron bearer를 검증하고, optional 결제 재조정 secret은 값이 있을 때만 형식을 검증한다. #206 workflow는 이 신규 secret을 provision/mutate하지 않는다. Preview에서는 이 키들이 없는 Fake-only 배포를 허용하되 신규 checkout은 항상 OFF로 고정한다. 실제 provider-backed canary는 #207의 rotated Korpay credential·공급사 취소 조율·직전 사용자 확인 뒤 최소 금액 1회로 한정한다.
...
- **결제 provider 전환** — provider-neutral 원장과 굿즈·티켓 `PaymentGateway` seam은 구현됐지만 실제 provider는 기본 OFF다. #207 Korpay dark deploy 뒤 신규 결제를 열며 Toss는 기존 거래 정리 전용으로만 유지한다(§9, #204~#207).
