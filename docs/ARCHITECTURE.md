...
| 스타일 | Tailwind **v4**. 공개 표면은 "White Catalog"([ADR-0011](adr/0011-lfs-storefront-redesign.md), 루트 `DESIGN.md`), 어드민은 editorial 토큰의 라이트 콘솔을 유지하고 `globals.css`의 어드민부는 그 아래 깔린 "Holographic Midnight" 유산 밑층이다 | 공개 `app/styles/wc-*.css`(토큰 진실원 `wc-foundation.css`), 어드민 `app/globals.css`·`app/styles/editorial-foundation.css`·`editorial-admin.css`·`admin-console.css`, 재조판 대기 표면 `about-legacy.css`·`offline-popups-legacy.css`·`legal-doc.css`, `postcss.config.mjs` |
