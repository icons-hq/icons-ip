...
| 배포 | 앱 전용 PR은 base SHA의 production migration+shared sync 성공 증거를 확인한 뒤 shared preview main을 변경 없이 읽고, merge-base 기준 diff에서 migration·roles·seed·Auth sync·Edge Function 또는 preview pipeline 변경을 찾으면 무데이터 `pr-<number>` branch에 migration·custom roles·seed와 변경된 Edge Functions를 적용한 뒤 선택 branch 키를 Vercel build/runtime에 동적으로 주입한다. rename은 원본·대상 경로를 모두 판정하도록 비탐지하고 hosted `config.toml` 전체 push는 제외한다. `main` push는 production Supabase linked migration·custom roles push 후 Vercel 원격 production build/deploy를 수행하고, 성공한 production migration 뒤 shared preview main을 같은 버전으로 맞춘다. PR close cleanup은 최종 base와 무관하게 같은 per-PR concurrency key에서 branch 생성을 기다린 뒤 deterministic branch만 지운다. Sensitive 환경변수는 Vercel build 안에서 검증하며 Vercel Git 자동 배포는 비활성화 | GitHub Secrets + `.github/workflows/pipeline.yml`, `vercel.json` |
...
- workflow가 Supabase Management API로 Site URL, 두 callback의 redirect allow-list, `mailer_otp_exp=3600`, secure email change와 email rate limit을 먼저 PATCH 후 read-back한다. Shared preview main의 Auth 설정은 main sync에서만 갱신하고 isolated branch는 생성 workflow에서 같은 preview URL 계약을 적용한다. Production Vercel 배포가 성공한 뒤에만 `supabase/templates/recovery.html` 원문을 production에 활성화·read-back한다. custom SMTP 필수 필드가 비어 있으면 production 배포를 실패시킨다.
