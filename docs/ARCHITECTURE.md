...
- **`preview_my_account_deletion` / `request_my_account_deletion` / `get_my_account_deletion_status`** — target user ID를 받지 않고 `auth.uid()`만 사용하는 self-only seam이다. 정확한 확인 문구와 UUID idempotency key를 요구하고 진행 주문·취소·환급·유효 티켓·staff 권한을 fail closed로 평가한다. 최초 요청 transaction에서 allowlist legal snapshot과 write fence를 함께 만들며, 기존 거래의 webhook·환급·배송 정합화 UPDATE는 막지 않는다. 성공 상태도 `awaiting_notification`까지만 표시하고 #191 intent, Storage/DB/Auth hard delete, #215 tombstone 전에는 완료를 반환하지 않는다.
