...
| 배포 | 앱 전용 PR은 base SHA의 production migration+shared sync 성공 증거를 확인한 뒤 shared preview main을 변경 없이 읽고, merge-base 기준 diff에서 Supabase 배포 또는 preview pipeline 변경을 찾으면 무데이터 `pr-<number>` branch에 migration·seed와 변경된 Edge Functions를 적용한 뒤 선택 branch 키를 Vercel build/runtime에 동적으로 주입한다. rename은 원본·대상 경로를 모두 판정하도록 비탐지한다. `main` push는 production Supabase linked migration push 후 Vercel 원격 production build/deploy를 수행하고, 성공한 production migration 뒤 shared preview main을 같은 버전으로 맞춘다. PR close cleanup은 최종 base와 무관하게 같은 per-PR concurrency key에서 branch 생성을 기다린 뒤 deterministic branch만 지운다. Sensitive 환경변수는 Vercel build 안에서 검증하며 Vercel Git 자동 배포는 비활성화 | GitHub Secrets + `.github/workflows/pipeline.yml`, `vercel.json` |
