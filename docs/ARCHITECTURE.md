...
- 실패·만료 복원: 승인 이력 없는 만료 `pending` 주문·예매는 pg_cron이 매분 `expire_stale_checkouts()`로 정리한다(승인 진행 중·모호 attempt 제외, 만료 후 5분 유예). 가상계좌는 Phase 1 결제수단(카드·간편결제) 밖이라 `DEPOSIT_CALLBACK` 구독·입금기한 예외 경로를 두지 않는다. 다만 `PAYMENT_STATUS_CHANGED`는 가상계좌에도 발송되므로, 위젯에 가상계좌가 노출되면 입금 뒤 `DONE` 웹훅이 reconcile을 태워 주문이 `paid`로 종결될 수 있다 — 그래서 어댑터는 `WAITING_FOR_DEPOSIT`을 만나면 입금 전에 즉시 전액 취소해 `declined`로 닫고(취소 실패는 `needs_review`), 토스 결제 어드민의 결제수단 비노출 확인은 이 코드 가드와 별개로 유지한다(런북 심사 트랙 — 가드가 없애지 못하는 것은 가상계좌를 골랐다가 곧바로 실패를 보는 구매자 경험과 취소 실패 건의 운영 처리다).
