...
- 결제·환급 재조정은 자동 scan/retry가 아니라 검토된 한 건만 지정하는 내부 route다. 분리된 `PAYMENT_RECONCILIATION_SECRET` exact bearer와 PII 없는 opaque `caseRef`를 요구하고, actor는 요청값을 신뢰하지 않고 서버가 `payment_reconciliation_service_v1`으로 고정해 DB audit에 operation·target·outcome을 남긴다. 관리자 아트워크 `CRON_SECRET`으로는 호출할 수 없다. #206 dark deploy에서는 secret을 provision하지 않아 route가 401로 닫히며 Preview/CI에는 어느 실 결제 자격 증명도 넣지 않는다.
