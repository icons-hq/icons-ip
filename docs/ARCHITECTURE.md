...
- 서버는 actor advisory lock 아래 claim을 `pending→processing`으로 먼저 전이한 뒤에만 staging 객체를 읽는다. actor별 동시 processing은 1개, 시작은 1분당 4회로 제한한다. 그 뒤 Sharp로 선언 MIME과 실제 JPEG/PNG/WebP 형식, 5MiB 이하, 축별 8192px 이하, 총 40MP 이하, 단일 프레임을 완전히 decode해 검증한다. 통과한 이미지는 같은 형식으로 다시 인코딩해 metadata와 부가 payload를 제거한 뒤 service role로 `public-media`에 promote하고 claim을 verified로 바꾼다. verified 전이는 만료를 최소 2시간 뒤로 밀어 운영자가 폼을 다 채우는 동안 클레임이 죽지 않게 한다 — 업로드 칸이 6개인 굿즈 폼에서 10분 창은 첫 이미지가 저장 전에 만료돼 트랜잭션 전체를 되돌린다.
