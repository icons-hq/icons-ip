...
- 서버 전용 env: Production legacy 정리용 `TOSS_SECRET_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `CRON_SECRET`, `EMAIL_PROVIDER_API_KEY`, `EMAIL_FROM`, `EMAIL_REPLY_TO`, `EMAIL_PROVIDER_ENDPOINT`, `SITE_URL`. 클라이언트 번들에 노출하지 않는다(`NEXT_PUBLIC_` 접두사 금지). Toss 공개 위젯 키·variant·production test override·키 지문은 신규 checkout 활성화 계약에서 제거했다.
...
- Vercel 환경변수는 sensitive로 유지한다. GitHub Actions는 값을 복호화할 수 없는 `vercel pull` + prebuilt 경로를 쓰지 않고 Vercel 원격 build를 요청한다. `prebuild` guard는 공통 필수 변수와 Production의 Toss known-only server key를 검증하고, Preview에서는 키 없는 Fake-only 배포를 허용하되 신규 checkout은 항상 OFF로 고정한다. 실제 provider-backed canary는 #207의 rotated Korpay credential·공급사 취소 조율·직전 사용자 확인 뒤 최소 금액 1회로 한정한다.
