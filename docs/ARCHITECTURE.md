...
화면↔라우트 매핑(현재):
`/`·`/ip`·`/ip/[id]`·`/shop`·`/shop/new`·`/shop/best`·`/shop/[goodId]`·`/search`·`/cart`·`/checkout`·`/checkout/[orderId]`·`/checkout/success`·`/checkout/fail`·`/orders`·`/orders/[orderId]`·`/packs`·`/binder`·`/games/[gameId]`·`/exchange`·`/market`·`/community`·`/events`·`/events/[eventId]`·`/offline-popups`·`/offline-popups/[eventId]`·`/ticket-checkout/[ticketOrderId]`·`/ticket-checkout/success`·`/ticket-checkout/fail`·`/tickets`·`/tickets/[ticketOrderId]`·`/notifications`·`/notifications/settings`·`/my`·`/my/wishlist`·`/my/coupons`·`/my/coins`·`/my/questions`·`/my/inquiries`·`/my/inquiries/new`·`/my/inquiries/[inquiryId]`·`/my/reviews`·`/my/reviews/new`·`/my/reviews/[reviewId]`·`/settings`·`/settings/delete-account`·`/legal/[document]`·`/about`·`/login`·`/onboarding`·`/update-password`·`/account-suspended`·`/admin`
...
- 실패·만료 복원: 승인 이력 없는 만료 `pending` 주문·예매는 pg_cron이 매분 `expire_stale_checkouts()`로 정리한다(승인 진행 중·모호 attempt 제외, 만료 후 5분 유예). 가상계좌는 Phase 1 결제수단(카드·간편결제) 밖이라 입금통보 웹훅·입금기한 예외 경로를 두지 않는다.
