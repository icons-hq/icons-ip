...
- confirm 결과는 추측하지 않는다. `200`+`DONE`이면서 orderId·통화·`totalAmount`·paymentKey가 attempt와 일치할 때만 `approved`이고, 타임아웃·5xx·409·`ALREADY_PROCESSED_PAYMENT`·미지 4xx는 조회 API로 실상태를 확인해 분기한다. 결정적 카드사·한도 거절 코드는 `declined`, 키·상점 설정 오류 코드는 `needs_review`, 인증 후 승인 시한(10분)을 넘긴 `NOT_FOUND_PAYMENT_SESSION`은 조회 404를 실패 확정으로 읽는다. 조회까지 모호하면 `unknown`으로 보존해 reconcile에 넘긴다. **조회 응답 자체가 401/403이거나 키·설정 오류 코드(`UNAUTHORIZED_KEY`·`INVALID_API_KEY`·`FORBIDDEN_REQUEST`·`API_KEY_ACCESS_DENIED` 등)면 일시 장애 `unknown`과 구분해 `needs_review`(reasonCode `provider_configuration_error`)로 격리한다** — confirm 재확인·reconcile·환불 조회 세 지점 모두 같은 규칙이다. 이 endpoint가 주문서형 시크릿 키를 허용하는지는 공식문서에 명문이 없어 실키 확인 전까지 미확인이며(API 키 문서의 `gsk` 허용 열거에 빠져 있다), 거부되면 조회에 기댄 모든 판정이 설정 오류로 드러나야 한다.
...
- 결제·환급 재조정은 자동 scan/retry가 아니라 검토된 한 건만 지정하는 내부 route다(`/api/internal/payments/tickets/reconcile`). 분리된 `PAYMENT_RECONCILIATION_SECRET` 자체와 exact bearer가 모두 16~128자 URL-safe 형식이어야 하며, PII 없는 opaque `caseRef`만 받는다. actor는 요청값을 신뢰하지 않고 서버가 고정해 DB audit에 operation·target·outcome을 남긴다. 관리자 아트워크 `CRON_SECRET`으로는 호출할 수 없다. 굿즈 재정합은 이 route가 아니라 웹훅이 트리거하는 같은 seam을 쓴다. 판매 pause는 해당 목적의 새 reserve·prepare·provider session 생성을 닫지만 이미 durable한 attempt의 known callback은 계속 drain한다. 알 수 없는 order·nonce는 DB claim에서 provider 호출 전에 거부한다.
