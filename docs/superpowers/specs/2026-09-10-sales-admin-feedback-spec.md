...
Q1~Q10은 사용자 답변을 반영했다. 사용자는 이후 추가 인터뷰 대신 현재 대화를 스펙으로 합성해 게시하도록 요청했다. 따라서 Q11/Q12와 아직 논의하지 않은 세부 규칙을 승인된 값으로 채우지 않는다. 아래는 누락하거나 범위에서 제외할 항목이 아니라 해당 구현의 선행 의존성이다.

| ID | 아직 확정되지 않은 사항 | 결정 전 지켜야 할 경계 |
|---|---|---|
| P01 | 지연 안내 채널(Q11): 기존 앱 알림·이메일 제안은 있었으나 답변 없음 | 채널과 수신 대상 계약 없이 실제 발송을 활성화하지 않음 |
| P02 | HTML 범위(Q12): 기본 문서 요소와 기존 HTML/CSS 디자인 이전 중 미답변 | 기본 편집 제안을 확정으로 취급하지 않음. 실행 가능한 삽입은 허용하지 않음 |
