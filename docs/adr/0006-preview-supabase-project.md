...
- 통합 브랜치처럼 base가 `main`이 아닌 PR은 현재 stage diff가 앱 전용이어도 누적된 선행 migration에 의존할 수 있으므로 항상 isolated를 쓴다. isolated branch는 parent의 현재 schema보다 오래된 PR을 조용히 superset으로 검수하지 않도록 PR head가 현재 repo `main`을 포함할 때만 만든다.
