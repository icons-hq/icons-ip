...
describe('GET /auth/recovery/callback', () => {
  ...
  it('rejects a legacy recovery code without exchanging a session', async () => {
    ...
    expect(response.headers.get('location')).not.toContain('recovery-code');
  ...
  it('normalizes an expired provider link before attempting verification', async () => {
    const response = await GET(request(
  ...
  it('rejects a missing token hash without trusting a query next path', async () => {
    const response = await GET(request(
  ...
  it('expires a verified recovery session when the current user cannot be revalidated', async () => {
    mocks.verifyOtp.mockImplementationOnce(async () => {
      mocks.setAll?.([
        {
          name: 'sb-local-auth-token',
