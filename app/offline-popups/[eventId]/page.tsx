...
export const metadata: Metadata = {
  title: '오프라인 팝업 상세 — ICONS',
  description: 'ICONS 오프라인 팝업 일정과 예매 가능한 회차를 확인하세요.',
};
...
export default async function Page({
  ...
  const next = `/offline-popups/${encodeURIComponent(event.id)}`;
  ...
  return (
    <EventDetail
      ...
      paymentAvailable={ticketCheckoutPaymentsEnabled(auth.user?.id)}
      sessions={sessions}
    />
  );
}
