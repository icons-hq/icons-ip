...
describe('POST /api/payments/confirm', () => {
  ...
  it('입금 전 티켓 가상계좌도 신규 Toss 경로에서는 provider 호출 전에 거절한다', async () => {
  ...
  it('신규 Toss 티켓 checkout은 대상 조회·provider 호출·원장 쓰기 전에 닫는다', async () => {
    const response = await POST(request(callbackBody({ orderId: TICKET_ORDER_ID })));
    ...
    await expect(response.json()).resolves.toEqual({
      error: {
        ...
        message: '신규 티켓 결제는 현재 결제 경로에서 진행할 수 없습니다.',
      },
    });
    expect(mocks.userTable).toBeNull();
    expect(mocks.confirm).not.toHaveBeenCalled();
    expect(mocks.rpc).not.toHaveBeenCalled();
    expect(mocks.upsert).not.toHaveBeenCalled();
