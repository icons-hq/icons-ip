-- S3 home redesign (#325): extend home_curations with the five new band kinds
-- (notice_strip, editor_pick, band_banner, best_tab, benefit), a per-kind jsonb
-- payload, and a slot column that distinguishes the two best_tab bands.
-- The three original kinds (hero, featured_ip, announcement) keep their rows,
-- constraints, and read contract unchanged.

alter table public.home_curations
  add column payload jsonb,
  add column slot text;

alter table public.home_curations
  drop constraint home_curations_kind_check,
  drop constraint home_curations_kind_ip_check,
  drop constraint home_curations_hero_image_check,
  add constraint home_curations_kind_check check (
    kind in (
      'hero', 'featured_ip', 'announcement',
      'notice_strip', 'editor_pick', 'band_banner', 'best_tab', 'benefit'
    )
  ),
  add constraint home_curations_kind_ip_check check (
    (kind = 'featured_ip' and ip_id is not null)
    or (kind <> 'featured_ip' and ip_id is null)
  ),
  -- Image-first kinds cannot render without artwork; the rest carry text only.
  add constraint home_curations_image_required_check check (
    kind not in ('hero', 'notice_strip', 'editor_pick', 'band_banner')
    or image_path is not null
  ),
  add constraint home_curations_payload_check check (
    payload is null or pg_catalog.jsonb_typeof(payload) = 'object'
  ),
  -- slot only disambiguates the two best_tab bands today. Widen deliberately
  -- with a later migration if another kind ever needs slots.
  add constraint home_curations_slot_check check (
    (kind = 'best_tab' and slot in ('category', 'popular'))
    or (kind <> 'best_tab' and slot is null)
  );

-- The upsert RPC gains target_slot / target_payload. Postgres cannot change a
-- function signature in place, so drop and recreate; the two new parameters
-- take defaults to keep every existing 11-argument caller working.
drop function public.admin_upsert_home_curation(
