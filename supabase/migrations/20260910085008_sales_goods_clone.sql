-- Existing artwork claims are single-use. A clone is allowed to reuse only
-- the locked source's exact paths on the newly-created target in this transaction.
-- No request payload or GUC can manufacture this private authorization.
create table private.goods_artwork_copy_authorizations (
  transaction_id bigint not null,
  target_good_id text not null references public.goods(id) on delete cascade,
  source_good_id text not null references public.goods(id) on delete cascade,
  actor_id uuid not null,
  allowed_paths text[] not null,
  primary key(transaction_id,target_good_id)
);
revoke all on private.goods_artwork_copy_authorizations from public,anon,authenticated,service_role;
create function private.can_copy_goods_artwork(target_good_id text,target_path text)
returns boolean language sql volatile security definer set search_path='' as $$
  select auth.uid() is not null and public.is_staff() and exists(
    select 1 from private.goods_artwork_copy_authorizations permission
    where permission.transaction_id=txid_current() and permission.target_good_id=can_copy_goods_artwork.target_good_id
      and permission.actor_id=auth.uid() and target_path=any(permission.allowed_paths));
$$;
revoke all on function private.can_copy_goods_artwork(text,text) from public,anon,authenticated,service_role;
grant execute on function private.can_copy_goods_artwork(text,text) to postgres;
...
-- #479: copy a goods registration into an independent draft.
-- The copy contract is deliberately explicit. New columns added later do not
-- become cloneable until this payload is reviewed and named here.
...
begin
  ...
  insert into private.goods_artwork_copy_authorizations(transaction_id,target_good_id,source_good_id,actor_id,allowed_paths)
    values(txid_current(),saved_id,source_good.id,actor_id,
      array_remove(array[source_good.image_path,source_good.detail_image_path]||coalesce(source_good.gallery_paths,'{}'),null));
  update public.goods set image_path=source_good.image_path,gallery_paths=source_good.gallery_paths,
    detail_image_path=source_good.detail_image_path where id=saved_id;
  ...
  values (
    ...
    pg_catalog.jsonb_build_object(
      ...
      'copiedArtwork',jsonb_build_object('imagePath',source_good.image_path,'galleryPaths',source_good.gallery_paths,'detailImagePath',source_good.detail_image_path),
