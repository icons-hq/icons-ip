 ...
 * is_staff readback 을 한 번 보내고, 로그아웃되면 값이 저절로 닫힌다.
 * presence 가 바뀔 때의 초기화는 effect 안 동기 setState 대신 렌더 중 "이전 값과 다르면
 * 되돌리기" 패턴으로 한다 — 같은 브라우저에서 스태프가 나가고 일반 회원이 들어와도 이전 결과가
 * 새 세션에 잠깐이라도 새지 않는다. 기본값은 항상 false 라 provider 밖(테스트·정적 렌더)에서도
 * 진입점은 닫혀 있다. */
...
export function SecondaryMarketDemoAvailabilityProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    void fetchSecondaryMarketDemoVisible().then((next) => {
      if (active && next) setVisible(true);
    });
    return () => {
      active = false;
    };
