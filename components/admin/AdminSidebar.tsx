...
export function AdminSidebar({
  ...
  const groupState = parseNavGroupState(useBrowserStoredValue(ADMIN_NAV_GROUPS_KEY));
  ...
  return (
    <aside className="admin-sidebar">
      ...
      <nav className="admin-nav">
        {groups.map((group) => {
          const collapsed = isNavGroupCollapsed(groupState, group.id, activeGroupId);
          return (
          <div key={group.id} className="admin-nav-group" data-collapsed={collapsed || undefined}>
            <NavGroupLabel
              ...
              onToggle={() => writeBrowserStoredValue(
                ADMIN_NAV_GROUPS_KEY,
                serializeNavGroupState(toggleNavGroup(groupState, group.id, activeGroupId)),
              )}
            />
            {/* 접힌 그룹의 화면도 그려 두고 CSS 로 숨긴다 — 900px 아래 상단 띠에서는 접힘과 무관하게
                전부 보여야 한다(띠에는 접는 단추가 없다). */}
...
/* 그룹 머리 = 접기 단추. 보고 있는 그룹도 접을 수 있다(PM 2026-09-09). */
