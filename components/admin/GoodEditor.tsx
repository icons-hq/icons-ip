...
import { ActionNotice, ErrorText, Field, SelectField, TextArea } from './fields';
...
function GoodEditorForm({ action, catalogIps, ipOptions, pending, selected, state, initialIpId, variants, origins, noticeDefaults, categories, shippingNoticeOptions, regionSummaries, formRef, onSubmitCapture }: {
  ...
  const recovered = Boolean(preservedFormValues(state, selected?.id));
  ...
  const attachForm = useCallback((form: HTMLFormElement | null) => {
    ...
    if (!form) return;
    if (baselineFingerprint.current === null) baselineFingerprint.current = goodEditorFingerprint(new FormData(form));
    return formRef(form, (data) => {
      // Native input listeners can run before React commits the option hidden input.
      // Editable options are authoritative here; never replace them with that older DOM value.
      setValues(current => ({ ...goodFormValues(data), variants: current.variants, variantBaseline: current.variantBaseline }));
      setDirty(recovered || goodEditorFingerprint(data) !== baselineFingerprint.current);
    });
  }, [formRef, recovered]);
  useEffect(() => {
  ...
  const setRows: Dispatch<SetStateAction<GoodsOptionRow[]>> = useCallback((update) => {
    setValues((current) => {
      const currentRows = restoreGoodsOptionRows(current.variants)!;
      const next = typeof update === 'function' ? update(currentRows) : update;
      return { ...current, variants: JSON.stringify(next) };
    });
  }, []);
  ...
  return <>
    ...
    <form action={action} className="wc-admin-kit col admin-good-workspace__form" onInvalidCapture={showInvalidFields} onSubmitCapture={submit} ref={attachForm} style={{ gap: 20 }}>
...
type GoodEditorProps = Omit<Parameters<typeof GoodEditorForm>[0], 'formRef' | 'onSubmitCapture'> & { accountId: string };

/** Own the entire edit/recovery session so callers cannot remount rows without their baseline. */
export function GoodEditor({ accountId, ...props }: GoodEditorProps) {
  const local = useAdminLocalAutosave({
    scope: { accountId, formId: 'good', recordId: props.selected?.id ?? null },
    fields: GOOD_LOCAL_DRAFT_FIELDS, serverState: props.state,
  });
  const state = withLocalRecoveryValues(props.state, props.selected?.id ?? null, local.snapshot.restoredValues);
  return <>
    <AdminLocalDraftNotice onDiscard={local.discard} onRestore={local.restore} pending={props.pending}
      recovery={Boolean(local.snapshot.recovery) && !preservedFormValues(props.state, props.selected?.id)} unavailable={local.snapshot.unavailable} />
    <GoodEditorForm {...props} state={state} formRef={local.formRef} onSubmitCapture={local.onSubmitCapture}
      key={`${local.scopeKey}:${props.state.attempt ?? 0}:${local.snapshot.revision}`} />
  </>;
}
