...
const SOURCE_LABELS: Record<AdminOrderTimelineSource, string> = {
  ...
  claim: '클레임', inquiry: '문의', email: '메일', note: '운영자 메모',
};
const ACTION_LABELS: Record<string, string> = {
  ...
  'admin.order.claim_approved': '클레임 승인', 'admin.order.claim_collected': '반송 상품 입고',
  'admin.order.claim_held': '클레임 보류', 'admin.order.claim_in_review': '클레임 검토 시작',
  'admin.order.claim_refund_completed': '클레임 환불 완료', 'admin.order.claim_refund_filed': '클레임 환불 접수',
  'admin.order.claim_rejected': '클레임 거부', 'admin.order.claim_reshipped': '교환 상품 발송',
  'admin.order.claim_resumed': '클레임 처리 재개',
};
...
function entryTitle(entry: AdminOrderTimelineEntry) {
  ...
  switch (entry.source) {
    ...
    case 'claim': return `${isOrderClaimType(entry.claimType) ? ORDER_CLAIM_TYPE_LABELS[entry.claimType] : '클레임'} 접수`;
...
function TimelineEntry({ entry }: { entry: AdminOrderTimelineEntry }) {
  ...
  return <li className="admin-order-detail__event">
    ...
    {entry.fromStatus && entry.toStatus ? <p>{orderStatus(entry.fromStatus)} → {orderStatus(entry.toStatus)}</p> : null}
    ...
    {href ? <Link href={href}>{entry.source === 'inquiry' ? '문의 상세 열기' : '클레임 상세 열기'}</Link> : null}
  </li>;
}
    ...
    <div className="admin-order-detail__overview">
      <AdminSectionCard title="주문 정보">
        <dl className="admin-order-detail__facts">
          ...
          <div><dt>주문자</dt><dd>{order.buyerName || '탈퇴 회원'}{order.buyerEmail ? <span>{order.buyerEmail}</span> : null}</dd></div>
      ...
      <AdminSectionCard title="배송 정보">
        <dl className="admin-order-detail__facts">
          ...
          <div><dt>현재 운송장</dt><dd>{order.trackingNumber ? `${order.shippingCarrier ?? ''} ${order.trackingNumber}` : '미등록'}</dd></div>
        </dl>
      </AdminSectionCard>
    </div>
    <AdminSectionCard title="주문 상품">
      <ul className="admin-order-detail__items">{items.map((item) => <li key={item.id}>
        <div><strong>{item.name}</strong><span>{item.type} · {item.qty}개</span></div>
