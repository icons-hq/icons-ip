...
/* 구 홈 화면은 hero·announcement·featuredIpIds 만 읽는다 — S3 에서 넓어진
   스냅샷의 신규 밴드 필드는 타입 충족용 빈 값으로 고정한다. */
const EMPTY_EXPANDED_CURATION: Pick<
  HomeCurationSnapshot,
  'heroSlides' | 'editorPicks' | 'goodsBands' | 'categoryBestTabs' | 'popularTabs' | 'benefitTiles'
> = {
  heroSlides: [],
  editorPicks: [],
  goodsBands: [],
...
function renderAboutLegacy(overrides: Partial<ComponentProps<typeof AboutLegacy>> = {}) {
  ...
  const props: ComponentProps<typeof AboutLegacy> = {
  ...
  return renderToStaticMarkup(<AboutLegacy {...props} />);
}
...
describe('AboutLegacy curation', () => {
  ...
  it('preserves the existing IP empty surface when no curation exists', () => {
    const html = renderAboutLegacy({
      catalog: catalog('supabase', []),
      curation: { ...EMPTY_EXPANDED_CURATION, hero: null, announcement: null, featuredIpIds: [] },
    });
  ...
  it('gives the announcement link explicit hover and focus-visible feedback', () => {
    ...
    const css = readFileSync(new URL('../../app/styles/about-legacy.css', import.meta.url), 'utf8');
  ...
  it('allows long unbroken hero and announcement titles to wrap without weakening Korean heading rules', () => {
    ...
    expect(cssForAboutLegacy()).toMatch(/\.icons-preview \.hero h1:not\(:has\(> span\)\)\s*\{[^}]*overflow-wrap:\s*break-word/);
    expect(cssForAboutLegacy()).toMatch(/\.icons-preview \.announcement strong\s*\{[^}]*overflow-wrap:\s*break-word/);
  });

  it('keeps Korean words intact and balances editorial headings', () => {
    expect(cssForAboutLegacy()).toMatch(
  ...
  it('renders a featured artwork override and keeps catalog key art when no override exists', () => {
    ...
    const artworkHtml = renderAboutLegacy({
      catalog: catalog('supabase', [artworkIp, fallbackIp]),
      curation: { ...EMPTY_EXPANDED_CURATION, hero: null, announcement: null, featuredIpIds: ['artwork', 'fallback'] },
    });
    const fallbackHtml = renderAboutLegacy({
      catalog: catalog('supabase', [fallbackIp]),
      curation: { ...EMPTY_EXPANDED_CURATION, hero: null, announcement: null, featuredIpIds: ['fallback'] },
    });
  ...
  it('treats nullable production post previews as an absent preview', () => {
    expect(() => renderAboutLegacy({
      postPreviewByIpId: { lumen: null } as unknown as ComponentProps<typeof AboutLegacy>['postPreviewByIpId'],
    })).not.toThrow();
  });
  ...
  it('hides film controls for one scene and renders one line per scene for two scenes', () => {
    ...
    const singleSceneHtml = renderAboutLegacy({ catalog: singleSceneCatalog, curation });
    const twoSceneHtml = renderAboutLegacy({
  ...
  it('keeps the original film lines centered with the source carousel contracts', () => {
    const source = readFileSync(new URL('./AboutLegacy.tsx', import.meta.url), 'utf8');
...
function cssForAboutLegacy() {
  return readFileSync(new URL('../../app/styles/about-legacy.css', import.meta.url), 'utf8');
}
...
describe('ICONS IP World Preview composition', () => {
  ...
  it('drops the self-owned chrome so the global shell keeps the header and footer', () => {
    const html = renderAboutLegacy();
    const css = cssForAboutLegacy();
  ...
  it('derives downstream experiences from the curated selected IP rather than a legacy featured IP', () => {
    ...
    const html = renderAboutLegacy({
      ...
      curation: { ...EMPTY_EXPANDED_CURATION, hero: null, announcement: null, featuredIpIds: ['lumen'] },
    });
