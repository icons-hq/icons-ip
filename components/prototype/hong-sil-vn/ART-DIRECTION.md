# 홍실 퀘스트 일러스트 제작 기록 (구판 · 재제작 대상)

> ⚠ **이 문서는 4막 구조 시절 원화 56컷의 제작 기록이다.** 8라운드 개편으로 슬롯이 74개가 됐고,
> 기존 56컷은 캐릭터 일관성·표정·무대 캐논 위반으로 **전량 재제작 대상**이 됐다.
> 현행 74컷의 생성 기준·슬롯별 장면·경로는 **[ART-V2-GENERATION.md](./ART-V2-GENERATION.md)** 에 기록했고,
> 이 파일은 구판을 무엇으로 교체했는지 비교하는 이력으로만 남긴다.
>
> 슬롯 ↔ 파일 연결의 현재 상태는 `art.ts`를 본다. 현행 74슬롯은 모두 v2 신규 원화로 연결됐고
> 구판 원화는 런타임에서 참조하지 않는다.

이 문서는 `story.ts`의 4막 시절 서사와 선택지 문구를 기준으로 만든 원화 56컷의 제작 기준이었다.
