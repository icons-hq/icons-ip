...
export function createBoxSession({ seed, rivals = 3, now = () => Date.now() }) {
  const st = {
    assign: buildRound(seed),
    cell: Array(CONFIG.slots).fill("open"),   // open | mine | rival | taken
  ...
  return {
    ...
    draw() {
      const out = st.picks.map((i) => {
        ...
        return { cell: i + 1, grade: p.g, name: p.name, value: p.value };
      });
 ...
 */
export function createDropSession({ seed, rivals = 3, now = () => Date.now() }) {
  const st = {
    assign: D.buildRound(seed),
    cell: Array(D.CONFIG.slots).fill("open"),
    today: 0,
  ...
  return {
    snapshot,
    canDrop: () => st.today < D.CONFIG.dailyLimit && D.leftCount(st) > 0,
    /** 투입 — 경로·착지 열·열린 칸을 한 번에 돌려준다. 화면은 이 값을 재생만 한다. */
    drop(entry) {
      const { path, col } = D.dropPath(entry, D.mulberry32((seed ^ (st.today + 1) * 2654435761) >>> 0));
      ...
      return { path, aimCol: col, col: settled, cell, grade: p.g, name: p.name, value: p.value };
    },
    tick() {
      const t = now();
      const events = [];
 ...
 */
export function createWireSession({ seed, rivals = 3, now = () => Date.now() }) {
  const st = {
    assign: W.buildRound(seed),
    rungs: W.buildLadder(seed, 0),       // 묶음마다 한 번. 묶음 안에서는 다시 짜지 않는다.
    block: 0,                             // 지금 몇 번째 배선인가
    cell: Array(W.CONFIG.slots).fill("open"),
  ...
  const snapshot = () => ({
    ...
    untilRewire: W.CONFIG.rewireEvery - (st.today % W.CONFIG.rewireEvery),
    today: st.today,
    left: W.leftCount(st),
  ...
  return {
    ...
    peek: (start) => (start in st.known ? st.known[start] : null),
    ...
    trace(start) {
      ...
      const nb = W.ladderBlock(st.today);
      const rewired = nb !== st.block;
      if (rewired) { st.block = nb; st.rungs = W.buildLadder(seed, nb); st.known = {}; }
      return { path, endCol: col, col: settled, cell, grade: p.g, name: p.name, value: p.value, rewired };
    },
    tick() {
      const t = now();
      const events = [];
 ...
 */
export function createDarkSession({ seed, rivals = 3, now = () => Date.now() }) {
  const st = {
    assign: K.buildRound(seed),
    cell: Array(K.CONFIG.slots).fill("open"),
  ...
  return {
    ...
    open(cell) {
      if (st.cell[cell] !== "open") return null;
      st.cell[cell] = "taken";
      st.today += 1;
      ...
      return { cell, grade: p.g, name: p.name, value: p.value };
    },
    tick() {
      const t = now();
      const events = [];
