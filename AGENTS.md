...
- 유료 가챠·`충전금`은 폐기됐다(ADR-0003·ADR-0004). `카드`는 `뽑기권`(UI 표기 "카드팩") 개봉과 참여형 게임의 무상 리워드로만 발급된다. 굿즈·티켓 신규 결제의 목표 provider는 Korpay다. 두 checkout은 provider-neutral seam으로 이동했지만 실제 provider gate는 OFF다. Toss는 `provider=toss`인 기존 거래의 조회·취소·웹훅에만 남긴다.
