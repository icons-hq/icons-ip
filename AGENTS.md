...
- 유료 가챠·`충전금`은 폐기됐다(ADR-0003·ADR-0004). `카드`는 `뽑기권`(UI 표기 "카드팩") 개봉과 참여형 게임의 무상 리워드로만 발급된다. 굿즈·티켓 신규 결제의 목표 provider는 Korpay다. 굿즈 checkout은 provider-neutral seam으로 이동했지만 실제 provider gate가 OFF이고, 티켓은 #206 전환 전까지 Toss checkout을 호환 유지한다. 전환 뒤 Toss는 `provider=toss`인 기존 거래의 조회·취소·웹훅에만 남긴다.
...
- 결제 callback body와 클라이언트 성공 신호는 확정의 진실원이 아니다. 굿즈 seam은 서버 전용 `PaymentGateway.confirm/reconcile` 결과와 DB 멱등 finalizer로만 확정하고, 현재 티켓 Toss 호환 경로는 웹훅 수신 뒤 provider 재조회 결과로 확정한다.
