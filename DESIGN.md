...
components:
  ...
  orders:         { status: 구현됨, ref: "app/orders/*; components/screens/Orders.tsx; components/screens/OrderDetail.tsx; components/orders/*; globals.css", note: "본인 주문 최신순 원장 + 상태·불변 굿즈 스냅샷·배송지·안전 결제/환불 요약·실제 카드팩 발급 상세·배송 전 취소. 데스크톱 영수증 2열, 모바일 1열" }
...
| 주문 내역 | `/orders`, `/orders/[orderId]` | 최신 주문 원장 → 상태·굿즈·배송지·결제·카드팩 상세 영수증 → 배송 전 취소/환불 상태 | 본인 `orders`/스냅샷 `order_items`/안전 결제·환불 컬럼/실제 `draw_tickets` + 취소 API |
