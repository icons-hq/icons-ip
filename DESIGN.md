...
components:
  ...
  settings:       { status: 구현됨, ref: "app/settings/*; components/screens/Settings.tsx", note: "서로 독립된 프로필·약관 form. 프로필 form은 private Storage signed image를 원형 아바타로 표시하고, 이미지가 없거나 서명에 실패하면 닉네임 첫 글자로 fallback. 닉네임·JPEG/PNG/WebP 아바타 편집과 기존 마케팅 동의 저장을 분리" }
...
| 설정 | `/settings` | 독립된 프로필·약관 form + 원형 signed-image 아바타(닉네임 첫 글자 fallback) | 본인 `profiles.nickname/avatar_path/consents` + private Storage signed URL |
