...
const actives: ActiveContentDefinition[] = [
  {
    ...
    name: text('기본 검격'),
    description: text('바라보는 방향의 적 무리를 길게 가르는 빠른 검격이다.'),
    behavior: [{ kind: 'pierce', target: 'facing', pattern: 'long-forward-cleave' }],
    levels: activeLevels(
      '기본 검격',
      [originalDesign],
      tuning({ cooldownTicks: 30, damage: 24, area: 560, speedPerTick: 1, durationTicks: 10, pierce: 5 }),
      { cooldownTicks: -2, damage: 8, area: 30, pierce: 2 },
    ),
  ...
  },
  {
    ...
    levels: activeLevels(
      '운룡등천',
      [originalDesign],
      tuning({ cooldownTicks: 70, damage: 40, area: 480, speedPerTick: 180, durationTicks: 90, pierce: 7 }),
      { cooldownTicks: -4, damage: 11, amount: 0.25, area: 45, pierce: 2 },
    ),
  ...
  },
  {
    ...
    levels: activeLevels(
      '빛의 검',
      [originalDesign],
      tuning({ cooldownTicks: 68, damage: 16, amount: 2, area: 360, speedPerTick: 180, durationTicks: 220, pierce: 4 }),
      { cooldownTicks: -3, damage: 5, amount: 0.75, area: 32, speedPerTick: 12, durationTicks: 25, pierce: 1 },
    ),
  ...
  },
  {
    ...
    levels: activeLevels(
      '용살검 그람',
      [originalDesign],
      tuning({ cooldownTicks: 88, damage: 76, area: 560, speedPerTick: 200, durationTicks: 85, pierce: 12, knockback: 36 }),
      { cooldownTicks: -5, damage: 18, area: 50, pierce: 2, knockback: 5 },
    ),
  ...
  },
  {
    ...
    levels: activeLevels(
      '낙뢰',
      [originalDesign],
      tuning({ cooldownTicks: 76, damage: 42, area: 460, speedPerTick: 1, durationTicks: 18, chainTargets: 6, knockback: 4 }),
      { cooldownTicks: -4, damage: 11, area: 40, chainTargets: 2 },
    ),
  ...
  },
  {
    ...
    levels: activeLevels(
      '블랙 드래곤 체인',
      [originalDesign],
      tuning({ cooldownTicks: 104, damage: 26, amount: 2, area: 520, speedPerTick: 260, durationTicks: 190, pierce: 7, knockback: -4 }),
      { cooldownTicks: -5, damage: 8, amount: 0.25, area: 55, durationTicks: 22, pierce: 2 },
    ),
...
const evolutions: EvolutionContentDefinition[] = [
  ['iron-wall-sword-path', '철벽검로', 'basic-sword-strike', 'wall-of-iron',
    ...
    tuning({ cooldownTicks: 20, damage: 120, amount: 2, area: 1_200, speedPerTick: 1, durationTicks: 16, pierce: 24, knockback: 48 })],
  ['swift-cloud-dragon', '신속운룡', 'cloud-dragon-ascent', 'hermes-secret-skill',
    ...
    tuning({ cooldownTicks: 42, damage: 88, amount: 3, area: 760, speedPerTick: 240, durationTicks: 120, pierce: 14, knockback: 24 })],
  ['purifying-light-sword', '정화광검', 'sword-of-light', 'purification-ring',
    [{ kind: 'orbit', target: 'radial', pattern: 'expanding-light-lattice' }],
    tuning({ cooldownTicks: 46, damage: 58, amount: 9, area: 620, speedPerTick: 300, durationTicks: 330, pierce: 8, knockback: 18 })],
  ['requiem', '레퀴엠', 'gram-dragon-slayer', 'dragon-heart',
    [{ kind: 'pierce', target: 'nearest', pattern: 'execution-line' }],
    tuning({ cooldownTicks: 68, damage: 250, amount: 2, area: 900, speedPerTick: 260, durationTicks: 110, pierce: 32, knockback: 72 })],
  ['regression-thunder', '회귀천뢰', 'lightning-fall', 'regressors-memory',
    [{ kind: 'chain', target: 'nearest-cluster', pattern: 'echoing-thunder-field' }],
    tuning({ cooldownTicks: 50, damage: 105, amount: 2, area: 760, speedPerTick: 1, durationTicks: 54, chainTargets: 15, knockback: 16 })],
  ['last-dragon-shackle', '최후의 용쇄', 'black-dragon-chain', 'last-human',
    ...
    tuning({ cooldownTicks: 72, damage: 82, amount: 4, area: 1_050, speedPerTick: 480, durationTicks: 320, pierce: 18, chainTargets: 6, knockback: -16 })],
...
export const tusinSurvivalPack = {
  ...
  contentVersion: 'tusin-survival-pack-v2',
  maxTicks: ticks(450),
  ...
  timeline,
  spawnLevelScaling: [
    { minimumPlayerLevel: 2, cadenceScale: 0.6, budgetScale: 1.5 },
  ],
  xpCurve,
  chestRules: {
