import { describe, expect, it, vi } from 'vitest';

/* 스태프 프리뷰 킬스위치 계약 — 상수 하나가 서버 게이트와 푸터 readback 을 함께 닫아야 한다.
 * staff 판정과 RPC 가 모두 참이어도 스위치가 꺼지면 어느 쪽도 열리지 않는다. 공개 스위치도
 * 꺼져 있으므로 이 조합은 "커뮤니티 완전 비공개" 로, 스위치를 내리기 전 상태와 같다. */
vi.mock('./community-visibility', () => ({
  COMMUNITY_ENABLED: false,
  COMMUNITY_STAFF_PREVIEW_ENABLED: false,
}));
vi.mock('@/lib/auth/admin', () => ({
  getCurrentAdminAuthState: async () => ({ isConfigured: true, user: null, role: 'admin', isStaff: true }),
}));
vi.mock('@/lib/supabase/client', () => ({
  createClient: () => ({ rpc: async () => ({ data: true, error: null }) }),
}));

import { canViewCommunity } from './community-visibility.server';
import { fetchCommunityStaffPreviewVisible } from './community-visibility.client';

describe('COMMUNITY_STAFF_PREVIEW_ENABLED = false', () => {
  it('서버 게이트와 푸터 readback 이 함께 닫힌다', async () => {
    await expect(canViewCommunity()).resolves.toBe(false);
    await expect(fetchCommunityStaffPreviewVisible()).resolves.toBe(false);
  });
});
