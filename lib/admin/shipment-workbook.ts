...
export interface ShipmentExportLine {
  ...
  goodCode:string;goodName:string;optionName:string;qty:number;deliveryNote:string;carrier:string;
}
...
export function parseShipmentExportColumns(value:unknown):ShipmentExportColumn[]|null {
  if(!Array.isArray(value)||value.length!==DEFAULT_SHIPMENT_EXPORT_COLUMNS.length)return null;
  const keys=new Set(DEFAULT_SHIPMENT_EXPORT_COLUMNS.map(column=>column.key));
  ...
  for(const raw of value){
    ...
    keys.delete(raw.key);headers.add(header);columns.push({key:raw.key,header});
  }
  return columns;
}
