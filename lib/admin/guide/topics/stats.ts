...
export const STATS_TOPIC: AdminGuideTopic = {
  ...
  sections: [
    ...
    {
      id: 'analysis',
      heading: '판매 분석 — 언제 · 누가 · 무엇이',
      paragraphs: [
        '한 화면에서 세 가지를 봅니다. 언제 팔렸는지(기간별 표), 누가 어디서 샀는지(축을 바꿔 가며), 무엇이 팔렸는지(굿즈 순위).',
        '셋이 같은 주문 묶음을 보기 때문에 축을 바꿔도 합계가 어긋나지 않습니다. 「결제수단으로 나눈 합」과 「지역으로 나눈 합」과 「기간 합계」가 항상 같습니다.',
      ],
      list: [
        '**매출이 잡히는 시각은 결제가 확정된 때**입니다. 카드는 주문한 때, 무통장은 입금을 확인한 때입니다 — 무통장은 두 시각이 며칠 벌어질 수 있습니다.',
        '**요일·시간대만 예외로 「주문한 때」를 봅니다.** 그건 매출이 아니라 고객이 언제 움직이는지를 보는 축이라 기준이 다릅니다.',
        '순매출 = 결제합계 − 환불합계입니다. 환불은 **환불이 끝난 날**에 잡히므로 지난달 숫자가 이번 달에 바뀌지 않습니다.',
        '지역은 우편번호 앞 두 자리로 읽습니다. 도서산간 판정도 같은 표를 쓰므로 배송비와 통계가 어긋나지 않습니다.',
        '비교 구간을 켜면 직전 기간이나 전년 동기와 나란히 봅니다. 비교 구간에 매출이 없으면 증감률을 계산하지 않습니다 — 0 대비 증가율은 뜻이 없습니다.',
        '화면 오른쪽 위의 「기준」 시각이 이 숫자를 언제 읽었는지 말합니다. 저장해 둔 값이 아니라 그때그때 세는 값이라, 환불 직후에 새로고침하면 바로 반영됩니다.',
      ],
      screens: [{ href: '/admin/stats/analysis' }],
    },
    {
      id: 'claims-stats',
      heading: '클레임 통계',
