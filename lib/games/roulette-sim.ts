...
function buildCourse(): CourseGeometry {
  const walls: Segment[] = [
    ...
    { x1: 0.3, y1: 22.5, x2: 6.9, y2: 24.0 },
    { x1: 9.7, y1: 25.7, x2: 3.1, y2: 27.2 },
    { x1: 0.3, y1: 28.9, x2: 6.9, y2: 30.4 },
    // 컬렉터 보울(체크포인트 1) — 전원이 고였다가 좁은 틈으로 빠져나간다.
    // 선두가 여기서 팩을 기다리므로 초반 격차가 리셋되고 순위가 재섞인다.
    { x1: 0.3, y1: 33.0, x2: 4.5, y2: 36.8 },
    { x1: 9.7, y1: 33.0, x2: 5.5, y2: 36.8 },
    // 깔때기 2(체크포인트 2) — 게이트 회전 막대가 출구를 주기적으로 봉쇄한다
    { x1: 0.3, y1: 47.0, x2: 4.3, y2: 50.0 },
    { x1: 9.7, y1: 47.0, x2: 5.7, y2: 50.0 },
    // 지그재그 선반 B(2단) — 최종 구간
    { x1: 9.7, y1: 60.5, x2: 3.0, y2: 62.0 },
    { x1: 0.3, y1: 63.5, x2: 7.0, y2: 65.0 },
  ];
  ...
  // 범퍼 — 고반발이라 맞으면 위로 튀어 오르기도 한다.
  // 마지막 두 개는 골 직전 디나이얼 존: 다 온 구슬을 되튕겨 "아깝다"를 만든다.
  const bumpers: Peg[] = [
    { x: 2.0, y: 40.8, r: 0.5 },
    { x: 5.0, y: 40.0, r: 0.5 },
    { x: 8.0, y: 40.8, r: 0.5 },
    ...
    { x: 1.8, y: 45.2, r: 0.5 },
    { x: 8.2, y: 45.2, r: 0.5 },
    // 디나이얼 존 — 벽 샛길을 막고 중앙 접근을 갈라 최후 관문을 만든다
    { x: 1.2, y: 66.3, r: 0.5 },
    { x: 8.8, y: 66.3, r: 0.5 },
    { x: 5.0, y: 65.8, r: 0.5 },
  ];
  ...
  const rotors: RotorDef[] = [
    { x: 3.2, y: 19.5, halfLength: 1.35, thickness: 0.28, omega: 1.9 },
    { x: 6.8, y: 20.3, halfLength: 1.35, thickness: 0.28, omega: -2.3 },
    // 보울 출구 게이트 — 출구(1.0m) 아래에서 돌며 낙하를 주기적으로 막고 쳐낸다
    { x: 5.0, y: 37.6, halfLength: 0.85, thickness: 0.3, omega: 1.5 },
    // 깔때기 2 게이트 — 수평이 되면 출구(4.3~5.7)가 완전히 닫힌다
    { x: 5.0, y: 51.0, halfLength: 0.9, thickness: 0.3, omega: 1.0 },
    // 골 앞 디나이얼 바 — 골인 직전 구슬을 쳐올려 "아깝다"와 대역전을 만든다
    { x: 5.0, y: 67.8, halfLength: 1.7, thickness: 0.28, omega: -2.4 },
  ];
