...
`iconsip.com`에서 사람 검토를 해야 할 때만, Vercel Production sensitive 환경변수에 Preview/로컬에서 검증한 결제위젯 세트의 `NEXT_PUBLIC_TOSS_CLIENT_KEY=test_gck_…`, `TOSS_SECRET_KEY=test_gsk_…`, 두 원문 값의 승인된 SHA-256인 `TOSS_PAYMENT_KEY_PAIR_SHA256`, 서버 전용 `ALLOW_TOSS_TEST_PAYMENTS_IN_PRODUCTION=true`, 검토 계정 UUID allowlist인 `TOSS_TEST_PAYMENT_REVIEWER_USER_IDS`를 함께 설정한다. override는 정확히 소문자 `true`여야 하며, 키 모드·지문·allowlist가 틀리거나 누락되면 fail closed로 결제를 열지 않는다. 이 기간에는 allowlist 계정만 주문·예매 생성과 승인·웹훅 확정을 진행할 수 있고, 그 밖의 계정이 우회해 만든 승인 건은 자동 취소한다. 테스트 결제는 실제 결제수단을 출금하지 않는다.

이 모드는 라이브 상점 검증을 대체하지 않는 임시 검토용이다. 검토 승인 뒤에는 `ALLOW_TOSS_TEST_PAYMENTS_IN_PRODUCTION`와 `TOSS_TEST_PAYMENT_REVIEWER_USER_IDS`를 제거하고, 동일한 결제위젯 세트의 `live_gck_…`/`live_gsk_…` 키와 그 지문을 복원한 다음 production 배포로 확인한다. 라이브 키는 override 없이 정상 동작한다.
