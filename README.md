...
- `pull_request`: `validate` 통과 후 같은 repo 브랜치 PR이면 merge-base 기준 전체 diff를 rename 비탐지로 읽어 preview DB mode를 고른다. Supabase 배포 변경이 없으면 base SHA의 main→shared sync 성공 증거를 확인한 뒤 shared main을 변경 없이 사용하고, 있으면 무데이터 `pr-<number>` branch를 재생성해 migration·seed·변경된 Edge Functions·baseline 검증을 마친 뒤 Vercel preview를 배포한다. fork PR은 secret 경계 때문에 preview 배포 없이 검증만 실행한다.
