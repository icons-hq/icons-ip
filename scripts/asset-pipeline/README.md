...
G2 급식실 버티컬 슬라이스의 플레이어·학생 좀비·김경미 보스 애니메이션과 급식실 세트를 검토 가능한 산출물로 만드는 파이프라인이다. `asset-spec.yaml`이 현재 생성 사양의 단일 정본이다. G2 산출물은 `docs/games/hyosan-memories-g2-assets/`에 게시되며 정확한 스펙·PNG·atlas 해시에 대한 사용자 승인 전에는 G3 Phaser 통합에 사용할 수 없다. 현재 G2 배치는 2026-08-28 사용자 승인을 통과했고 정본은 `docs/games/hyosan-memories-g2-assets/approval-record.json`이다. 파이프라인을 다시 실행하거나 승인된 해시가 달라지면 manifest는 다시 `pending-user-approval`이 되며 재승인이 필요하다.
...
- 프레임별 보이는 bbox를 검사한 뒤 전체 시트에서 가장 큰 bbox 기준의 공통 scale과 bottom-center 발 anchor를 적용한다. 빈 셀은 즉시 실패한다.
